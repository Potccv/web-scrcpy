 ...
 *   - h265web:h265web.js 播放器(硬解优先 + WASM 软解回退,H.264/H.265/AV1)
 ...
 * 解码方式下拉框按“编码格式”动态生成,每个编解码器只展示自己可用的解码项。
 */
...
import { H265webDecoder } from "./h265web.js";

export const DECODER_OPTIONS = [
  { id: "auto", label: "自动选择" },
  { id: "webcodecs", label: "WebCodecs(浏览器原生)" },
  { id: "h265web", label: "h265web.js(硬解+软解)" },
...
const DECODER_LABELS = {
  auto: "自动选择",
  webcodecs: "WebCodecs(浏览器原生)",
  "h265web": "h265web.js(硬解+软解)",
  "custom-js": "自定义JS/WASM解码",
  mse: "MediaSource(延迟高,不推荐)",
};

/**
 * 根据编码格式返回可选的解码方式列表(每个编解码器单独展示对应解码项)。
 */
export function getDecoderOptions(codec) {
  const options = [
    { id: "auto", label: "自动选择" },
    { id: "webcodecs", label: "WebCodecs(浏览器原生)" },
  ];
  if (H265webDecoder.supported(codec)) {
    options.push({ id: "h265web", label: h265webDecoderLabel(codec) });
  }
  if (codec === "h264" && BroadwayDecoder.supported(codec)) {
    options.push({ id: "custom-js", label: "自定义JS(Broadway)" });
  } else if (codec === "h265" && Libde265Decoder.supported(codec)) {
    options.push({ id: "custom-js", label: "自定义JS(libde265)" });
  }
  if (MseDecoder.supported(codec)) {
    options.push({ id: "mse", label: "MediaSource(延迟高,不推荐)" });
  }
  return options;
}
...
export function h265webDecoderLabel(codec) {
  return `h265web.js(${codec.toUpperCase()})`;
}

/**
 * 创建解码器实例。
 * @param {string} decoderId 'webcodecs' | 'mse' | 'custom-js' | 'h265web'
 * @param {object} opts 含 codec(必填),其余为 {canvas, videoEl, onFrame, onError, onInfo, mediaToken}
 */
export function createDecoder(decoderId, opts) {
  switch (decoderId) {
    ...
    case "h265web":
      return new H265webDecoder(opts);
    case "custom-js":
...
export async function decoderSupports(decoderId, codec) {
  switch (decoderId) {
    ...
    case "h265web":
      return H265webDecoder.supported(codec);
    case "custom-js":
 ...
 *
 * @param {string} userChoice 'auto' | 'webcodecs' | 'mse' | 'custom-js' | 'h265web'
 * @param {string} codec h264/h265/av1
...
export async function resolveDecoder(userChoice, codec) {
  // 自动选择:WebCodecs 优先,其次 h265web.js(硬解+软解),再自定义JS,MediaSource 最后
  const candidates =
    userChoice === "auto" ? ["webcodecs", "h265web", "custom-js", "mse"] : [userChoice];
  ...
  for (const id of candidates) {
    if (await decoderSupports(id, codec)) {
      ...
      if (id === "custom-js") {
        ...
        label = h265webDecoderLabel(codec);
        warning =
          codec === "av1"
            ? "h265web.js(AV1) 需要 Chrome 且设备端支持 AV1 编码;若无法出图请改用 WebCodecs"
            : `h265web.js(${codec.toUpperCase()}) 硬解优先,WASM 软解回退`;
      }
  ...
  const hint = {
    ...
    h265: "H.265 需要浏览器 WebCodecs(HEVC)支持(如 Chrome 硬件解码、Safari),或使用「h265web.js/自定义JS解码」",
    av1: "AV1 需要浏览器 WebCodecs 支持(Chrome/Firefox/Edge),或使用「h265web.js」(Chrome)",
  }[codec];
 ...
 * @returns {Promise<{webcodecs: object, h265web: object, mse: object, customJs: object}>}
 *         每项为 {codec: boolean} 映射
 */
export async function probeSupport() {
  const result = { webcodecs: {}, h265web: {}, mse: {}, customJs: {} };
  const tasks = [];
  for (const codec of VIDEO_CODECS) {
    tasks.push(
      decoderSupports("webcodecs", codec).then((v) => (result.webcodecs[codec] = v)),
      decoderSupports("h265web", codec).then((v) => (result.h265web[codec] = v)),
